    ...
    >
      <nav className="flex items-center justify-between w-full mx-auto px-margin-mobile md:px-margin-desktop max-w-[1440px]">
        <div className="flex items-center gap-4 md:gap-8">
          ...
          <Link to="/" className="text-xl font-extrabold tracking-widest uppercase text-primary">
            ELÉVATION
          </Link>
          ...
          <div className="hidden gap-6 md:flex">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className="transition-colors font-label-caps text-label-caps text-on-surface-variant hover:text-secondary"
              >
                {link.name}
              </Link>
            ))}
        ...
        <div className="flex items-center gap-4 md:gap-6">
          <div ref={searchRef} className="relative flex items-center">
            ...
            <button
              ...
              className="flex items-center justify-center transition-colors hover:text-secondary"
            >
          ...
          >
            <span className="transition-colors material-symbols-outlined group-hover:text-secondary">
              shopping_bag
            </span>
            {cartCount > 0 && (
      ...
      <div
        className={`absolute top-full left-0 w-full bg-surface/95 backdrop-blur-md overflow-hidden transition-all duration-300 md:hidden ${
          isMobileMenuOpen
            ? 'max-h-64 border-b border-outline-variant/30 py-4'
            : 'max-h-0 py-0'
        }`}
      >
        <div className="flex flex-col space-y-4 px-margin-mobile">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              className="text-sm tracking-widest uppercase font-label-caps text-primary"
